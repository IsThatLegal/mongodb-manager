...
class ConfigManager {
  constructor(configPath = null, options = {}) {
    this.serverless = options.serverless || false;
  ...
  async load() {
    // In serverless mode, skip file-based config and use default
    if (this.serverless) {
      console.log('Serverless mode: using in-memory configuration');
      return this.config;
    }
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        // In serverless, don't try to create files
        if (this.serverless) {
          console.log('Serverless mode: config file not found, using defaults');
          return this.config;
        }
