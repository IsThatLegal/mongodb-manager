...
class ClusterManager extends EventEmitter {
  ...
  async addCluster(name, clusterConfig) {
    try {
      ...
      console.log(`✅ Connected to cluster ${name}`);
      
      await this.testConnection(client);
      console.log(`✅ Connection test passed for ${name}`);
      ...
      try {
        ...
        const dbStats = await admin.command({ listDatabases: 1 });
        
        clusterInfo = {
          version: buildInfo.version,
          databases: dbStats.databases.map(db => db.name),
          totalDatabases: dbStats.databases.length,
          gitVersion: buildInfo.gitVersion
        };
        console.log(`✅ Retrieved cluster info for ${name}:`, clusterInfo.databases.length, 'databases');
      } catch (infoError) {
        console.log(`⚠️  Could not retrieve cluster info for ${name}:`, infoError.message);
      }
      
      this.connections.set(name, {
        client,
        config: { ...clusterConfig, clusterInfo },
  ...
  listClusters() {
    ...
    return Array.from(allClusterNames).map(name => {
      ...
      if (connection) {
        // Connected cluster
        const clusterInfo = connection.config.clusterInfo || {};
        return {
          name,
          environment: connection.config.environment || 'unknown',
          databases: clusterInfo.databases || connection.config.databases || [],
          status: health.status,
          connectedAt: connection.connectedAt,
          lastHealthCheck: health.lastCheck,
          version: clusterInfo.version,
          totalDatabases: clusterInfo.totalDatabases,
